<template>
  <div id="roundend" class="screen active">
    ...
    <div class="re-headline">{{ store.currentTeam.name.toUpperCase() }} DONE!</div>
    ...
    <div class="scores-grid">
      ...
      >
        ...
        <div :class="['sr-score', t.score === maxScore && maxScore > 0 ? 'top' : '']">{{ t.score }}</div>
      </div>
    </div>

    <div class="re-earned">
      <div class="re-earned-label">{{ store.currentTeam.name }} scored this round</div>
...
const nextTeamName = computed(() => store.teams[nextTeamIdx.value]?.name.toUpperCase() || '')
